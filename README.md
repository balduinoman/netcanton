...
1. Navigate to your extracted Canton source directory.
2. Create a `Dockerfile`. You can find baseline templates within the source examples or the official documentation.
...
1. Create a `config` directory in your root project folder.
2. Populate this directory with four files: `mediator.conf`, `sequencer.conf`, `participant1.conf`, and `participant2.conf`.
3. Ensure each file contains the specific configuration block relevant to that component (e.g., unique storage ports and ledger API settings) as defined in the source documentation.

### 2.3 Network Orchestration

Create a `docker-compose.yml` file to define the network topology. This orchestration should include:
